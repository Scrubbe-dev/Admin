...
const email_integration_service_1 = require("../modules/3rd-party-configurables/email-integration/email-integration.service");
...
async function pollInbox() {
    const connection = await imap_simple_1.default.connect({
        imap: {
            user: imapUser,
            password: imapPass,
    ...
    setInterval(async () => {
        ...
        for (let res of results) {
            ...
            try {
                const result = await emailIntegrationService.handleInboundEmail(payload);
...
const callhandleInboundEmail = async () => {
    const result = await emailIntegrationService.handleInboundEmail({
        ...
        from: "e.ofoneta@scrubbe.com",
        subject: "Raise: incident for bad connection",
        body: "reason: Regular login attempt from different locations\n" +
            "            assignTo: ebubeofoneta@gmail.com\n" +
            "            username: margarete\n" +
            "            priority: LOW\n" +
            "            template: NONE\n",
