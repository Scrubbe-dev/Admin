...
exports.IMSService = void 0;
const client_1 = require("@prisma/client");
const ims_utils_1 = require("./ims.utils");
const prisma = new client_1.PrismaClient();
class IMSService {
    /**
     * Setup IMS for a new company
     */
    static async setupIMS(userId, requestData) {
        try {
            ...
            return await prisma.$transaction(async (tx) => {
                ...
                return {
                    ...
                    domain: (0, ims_utils_1.generateDomain)(requestData.companyName)
                };
            });
        }
        catch (error) {
     ...
     */
    static validateIMSSetupRequest(data) {
        const errors = [];
        ...
        if (!Array.isArray(data.inviteMembers)) {
            ...
            data.inviteMembers.forEach((member, index) => {
                ...
                if (!Array.isArray(member.accessPermissions) || member.accessPermissions.length === 0) {
                    ...
                    member.accessPermissions.forEach((permission) => {
                        if (!Object.values(client_1.AccessPermissions).includes(permission)) {
