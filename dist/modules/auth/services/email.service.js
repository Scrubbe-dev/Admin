...
class EmailService {
    ...
    async sendVerificationEmail(email, code) {
        try {
            ...
            await this.transporter.sendMail({
                from: nodemailer_config_1.emailConfig.from,
                to: email,
    ...
    async sendWarRoomEmail(from, to, incidentTicket, meetingLink) {
        try {
            await this.transporter.sendMail({
                from,
                to,
    ...
    `,
            ...
            throw new Error(`Failed to send war room email: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
