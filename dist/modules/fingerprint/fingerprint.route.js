...
const fingerprintRouter = express_1.default.Router();
const tokenService = new token_service_1.TokenService(process.env.JWT_SECRET, process.env.JWT_EXPIRES_IN || "1h", 15 // in mins
);
const fingerprintService = new fingerprint_service_1.FingerprintService();
const fingerprintController = new fingerprint_controller_1.FingerprintController(fingerprintService);
 ...
 */
fingerprintRouter.post("/configuration", authMiddleware.authenticate, (req, res, next) => {
 ...
 *         description: Internal server error
 */
fingerprintRouter.get("/configuration", authMiddleware.authenticate, (req, res, next) => {
    fingerprintController.getUserFingerprintConfig(req, res, next);
});
exports.default = fingerprintRouter;
