...
class EmailAnalyzer {
    ...
    async analyzeEmail(request) {
        ...
        try {
            ...
            // Sender checks
            const senderChecks = this.checkDisplayNameSpoofing(request.displayName, request.senderEmail);
            analysis.sender_analysis.findings.push(...senderChecks);
            analysis.sender_analysis.is_suspicious = senderChecks.length > 0;
            // Content checks
            const contentChecks = this.checkContentIndicators(request.subject, request.content);
