...
class PdfService {
     ...
     */
    static async generateIncidentReportPdf(incidentId, description) {
        try {
            ...
            error_util_1.ErrorUtil.logError('PdfService.generateIncidentReportPdf', error);
     ...
     */
    static async fetchIncidentData(incidentId, description) {
        const incident = await prisma_1.default.incidentTicket.findFirst({
            where: { id: incidentId },
            include: {
        ...
        if (!incident)
            return null;
        const mainIncident = await prisma_1.default.incident.findFirst({
        ...
        if (incident.assignedToEmail) {
            incidentUser = await prisma_1.default.user.findFirst({
                where: { email: incident.assignedToEmail }
            });
        }
        const resolveIncident = await prisma_1.default.resolveIncident.findFirst({
            where: { incidentTicketId: incident.id }
        });
            ...
            actionItems: [
                {
                    ...
                    dueDate: resolveIncident?.followUpDueDate,
     ...
     */
    static async generateMockTimeline(incident) {
        const timeline = [
            {
     ...
     */
    static checkPageBreak(doc, requiredSpace = 60) {
     ...
     */
    static generatePdfFromIncidentData(incidentData, description) {
     ...
     */
    static addIncidentReportContent(doc, incident, description) {
     ...
     */
    static addIncidentHeader(doc, incident, pageWidth) {
        ...
        doc.fillColor('white')
            ...
            .font('Helvetica-Bold')
            .text('INCIDENT POST-MORTEM REPORT', this.DESIGN.margins.content, startY + 15, { width: pageWidth - 40, align: 'center' });
        ...
        doc.fontSize(this.DESIGN.fonts.heading)
            ...
            .text(`${incident.ticketId} - ${incident.priority} Priority`, this.DESIGN.margins.content, startY + 50, { width: pageWidth - 40, align: 'center' });
        // Key metadata row
        const metaY = startY + 80;
        doc.fontSize(this.DESIGN.fonts.small)
            .fillColor('#e2e8f0');
        // Date created
        doc.text(`Created: ${incident.createdAt.toLocaleString()}`, this.DESIGN.margins.content, metaY);
        // Status
        doc.text(`Status: ${incident.status}`, this.DESIGN.margins.content + 200, metaY);
        // Duration
        if (incident.resolvedAt) {
            const duration = Math.floor((incident.resolvedAt.getTime() - incident.createdAt.getTime()) / (1000 * 60));
            doc.text(`Duration: ${duration} minutes`, this.DESIGN.margins.content + 350, metaY);
        }
     ...
     */
    static addExecutiveSummary(doc, incident, pageWidth, description) {
        ...
        doc.fontSize(this.DESIGN.fonts.body)
            ...
            .text(summaryText, this.DESIGN.margins.content, doc.y, {
            width: pageWidth - 40,
            align: 'justify',
            lineGap: 4
        });
     ...
     */
    static addBusinessImpactBox(doc, impact, pageWidth) {
        this.checkPageBreak(doc, 100);
        const boxHeight = 80;
        ...
        doc.fontSize(this.DESIGN.fonts.small)
            .fillColor(this.DESIGN.colors.text)
            .font('Helvetica');
        doc.text(`• Users Affected: ${impact.usersAffected.toLocaleString()}`, this.DESIGN.margins.content + 15, metricsY);
        doc.text(`• Downtime: ${impact.downtime} minutes`, this.DESIGN.margins.content + 15, metricsY + 12);
        if (impact.financialImpact) {
            doc.text(`• Est. Financial Impact: $${impact.financialImpact.toLocaleString()}`, this.DESIGN.margins.content + 15, metricsY + 24);
        }
     ...
     */
    static addIncidentTimeline(doc, incident, pageWidth) {
        this.checkPageBreak(doc, 100);
     ...
     */
    static addRootCauseAnalysis(doc, incident, pageWidth) {
        this.checkPageBreak(doc, 100);
        ...
        if (incident.rootCause) {
            ...
            doc.fontSize(this.DESIGN.fonts.body)
                .font('Helvetica')
                .fillColor(this.DESIGN.colors.text)
                .text(incident.rootCause.technical, this.DESIGN.margins.content, doc.y, {
                width: pageWidth - 40,
                align: 'justify'
            });
     ...
     */
    static addImpactAnalysis(doc, incident, pageWidth) {
        this.checkPageBreak(doc, 80);
     ...
     */
    static addResolutionDetails(doc, incident, pageWidth) {
        this.checkPageBreak(doc, 100);
        ...
        if (incident.resolutionDetails) {
            ...
            doc.fontSize(this.DESIGN.fonts.body)
                .font('Helvetica')
                .fillColor(this.DESIGN.colors.text)
                .text(incident.resolutionDetails.permanentFix || 'N/A', this.DESIGN.margins.content, doc.y, {
                width: pageWidth - 40,
                align: 'justify'
            });
            // Workarounds
     ...
     */
    static addLessonsLearned(doc, incident, pageWidth) {
        this.checkPageBreak(doc, 100);
     ...
     */
    static addActionItems(doc, incident, pageWidth) {
        this.checkPageBreak(doc, 100);
    ...
    // Helper methods
    static addSectionHeader(doc, title, pageWidth) {
        doc.fontSize(this.DESIGN.fonts.heading)
            .font('Helvetica-Bold')
    ...
    static drawTableHeader(doc, headers, colWidths, y, pageWidth) {
    ...
    static drawTableRow(doc, rowData, colWidths, y) {
        let currentX = this.DESIGN.margins.content;
        doc.fontSize(this.DESIGN.fonts.small)
            .font('Helvetica')
            .fillColor(this.DESIGN.colors.text);
     ...
     */
    static async generatePdf(request) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                error_util_1.ErrorUtil.logError('PdfService.generatePdf', error);
     ...
     */
    static addProfessionalContent(doc, request) {
     ...
     */
    static addProfessionalHeader(doc, pageWidth) {
        ...
        doc.fillColor('white')
            ...
            .font('Helvetica-Bold')
            .text(this.COMPANY_BRANDING.companyName, this.DESIGN.margins.content, startY + 15, { width: pageWidth - 40, align: 'left' });
        doc.fontSize(this.DESIGN.fonts.small)
            .font('Helvetica')
            .fillColor('#e2e8f0')
            .text(this.COMPANY_BRANDING.address, this.DESIGN.margins.content, startY + 35, { width: pageWidth - 40, align: 'left' });
        doc.text(`${this.COMPANY_BRANDING.phone} | ${this.COMPANY_BRANDING.email}`, this.DESIGN.margins.content, startY + 50, { width: pageWidth - 40, align: 'left' });
        doc.y = startY + headerHeight + this.DESIGN.spacing.section;
    }
    /**
     * Add document title with professional styling (legacy)
     */
    static addDocumentTitle(doc, metadata, pageWidth) {
        const title = metadata.title || 'Official Document';
        doc.fontSize(this.DESIGN.fonts.title)
            .font('Helvetica-Bold')
            .fillColor(this.DESIGN.colors.primary)
            .text(title, this.DESIGN.margins.content, doc.y, { width: pageWidth - 40, align: 'center' });
     ...
     */
    static addEnhancedMetadata(doc, id, metadata, pageWidth) {
        const boxHeight = 100;
        const startY = doc.y;
     ...
     */
    static addMetadataField(doc, label, value, x, y) {
        doc.fontSize(this.DESIGN.fonts.small)
            .font('Helvetica-Bold')
            .fillColor(this.DESIGN.colors.secondary)
     ...
     */
    static addMainContent(doc, description, pageWidth) {
        this.checkPageBreak(doc, 80);
        doc.fontSize(this.DESIGN.fonts.heading)
            .font('Helvetica-Bold')
            .fillColor(this.DESIGN.colors.primary)
            .text('Document Content', this.DESIGN.margins.content, doc.y, { width: pageWidth - 40 });
        const lineY = doc.y + this.DESIGN.spacing.micro;
        doc.strokeColor(this.DESIGN.colors.accent)
            .lineWidth(3)
            .moveTo(this.DESIGN.margins.content, lineY)
        ...
        doc.fontSize(this.DESIGN.fonts.body)
            .font('Helvetica')
            .fillColor(this.DESIGN.colors.text)
            .text(description || 'No content available', this.DESIGN.margins.content, doc.y, {
            width: pageWidth - 40,
            align: 'justify',
            lineGap: 4,
