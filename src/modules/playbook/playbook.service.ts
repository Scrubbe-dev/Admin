...
import { AppError } from './playbook.util';
import { RecommendedPlaybooksResponse } from './playbook.type';

const prisma = new PrismaClient();

export class PlaybookService {
  static async getRecommendedPlaybooks(ticketId: string): Promise<RecommendedPlaybooksResponse> {
    ...
    const playbooks = ticket.playbookRecommendations.map(recommendation => ({
      ...
      steps: recommendation.playbook.steps as string[], // Assuming steps is stored as an array of strings
  ...
  static async generatePlaybookRecommendations(ticketId: string): Promise<RecommendedPlaybooksResponse> {
    // Find the ticket
    const ticket = await prisma.incidentTicket.findUnique({
      where: { id: ticketId },
      include: {
