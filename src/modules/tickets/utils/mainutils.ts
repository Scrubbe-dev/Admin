...
export class TicketHistoryUtils {
  static async getTicketHistory(ticketId: string) {
    ...
    const statusChanges = await prisma.incidentTicket.findUnique({
      where: { id: ticketId },
      select: {
        status: true,
        createdAt: true,
    ...
    // Add comment events
    comments.forEach((comment: { createdAt: any; content: any; author: { firstName: any; lastName: any; }; }) => {
      historyEvents.push({
