...
export class FraudDetectionService {
  ...
  public async analyzeTransaction(
    ...
    const requestId = `fraud-req-${uuidv4().substring(0, 10)}`;
    ...
    try {
      ...
      logger.error('Fraud analysis failed', { requestId, error });
      throw error;
    }
  }
  ...
  private async detectRiskFactors(
    ...
    if (ipMismatch) riskFactors.push(ipMismatch);
    ...
    if (addressMismatch) riskFactors.push(addressMismatch);

    // 4. Check for unusual payment methods
    const paymentMethodRisk = this.checkPaymentMethodRisk(request);
    if (paymentMethodRisk) riskFactors.push(paymentMethodRisk);
  ...
  private checkIPLocationMismatch(request: FraudDetectionRequest): RiskFactor | null {
  ...
  private checkTransactionVelocity(request: FraudDetectionRequest): RiskFactor | null {
  ...
  private checkAddressMismatch(request: FraudDetectionRequest): RiskFactor | null {
  ...
  private checkPaymentMethodRisk(request: FraudDetectionRequest): RiskFactor | null {
  ...
  private checkDeviceRisk(request: FraudDetectionRequest): RiskFactor | null {
  ...
  private generateFraudAssessment(
